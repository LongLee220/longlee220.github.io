#### Publications  
(+ indicates equal contribution, * indicates corresponding author)

- <strong>Longlong Li+</strong>, Yipeng Zhang+, Guanghui Wang, and Kelin Xia*.  
  Kolmogorov–Arnold Graph Neural Networks for Molecular Property Prediction.  
  <strong>Nature Machine Intelligence</strong>, 2025.  
  [[Paper]](https://www.nature.com/articles/s42256-025-01087-7)

- Haixia Hu+, …, <strong>Longlong Li</strong> (15/33 authors), …, Hang Yin*.  
  Revealing electron transport connectivity as an important factor influencing stability of organic solar cells.  
  <strong>Nature Communications</strong>, 2025.  
  [[Paper]](https://www.nature.com/articles/s41467-025-60599-3)

- Yipeng Zhang+, <strong>Longlong Li+</strong>, and Kelin Xia*.  
  Rhomboid Tiling for Geometric Graph Deep Learning.  
  <strong>International Conference on Machine Learning (ICML)</strong>, 2025.  
  [[Paper]](https://openreview.net/forum?id=lTZEAqL1R1)

- Bingqing Han+, Yipeng Zhang, <strong>Longlong Li</strong>, Xinqi Gong*, and Kelin Xia*.  
  TopoQA: a topological deep learning-based approach for protein complex structure interface quality assessment.  
  <strong>Briefings in Bioinformatics</strong>, 2025.  
  [[Paper]](https://academic.oup.com/bib/article/26/2/bbaf083/8068116)


#### Preprints

- <strong>Longlong Li+</strong>, Mengyang Zhao, Guanghui Wang, and Cunquan Qu*.  
  Multi-Scale Harmonic Encoding for Feature-Wise Graph Message Passing.  
  <strong>arXiv</strong>, 2025.  
  [[arXiv]](https://arxiv.org/abs/2505.15015)

- <strong>Longlong Li+</strong>, Cunquan Qu*, and Guanghui Wang.  
  TGT: A Temporal Gating Transformer for Smartphone App Usage Prediction.  
  <strong>arXiv</strong>, 2025.  
  [[arXiv]](https://arxiv.org/abs/2502.16957)
